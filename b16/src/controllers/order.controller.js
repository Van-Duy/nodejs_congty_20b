...
const { create: createOder, findOne: findOneOrder, getAllByParams } = require("../services/order.services")
...
const getOrderByCode = async ( req,res) => {
    ...
    let order = await findOneOrder({ code })
    if (!order) throw new BAD_REQUEST_ERROR("Not found by code")

    let user = await findUserById(req.userId)
    if (!user) throw new BAD_REQUEST_ERROR("User not null")


    if (user._id.toString() != order.id_user.toString()) throw new BAD_REQUEST_ERROR("Not found by code")
...
const history = async ( req,res) => {
    ...
    new Ok({
        ...
        metadata: await getAllByParams({ id_user : req.userId })
    }).send(res)
...
module.exports = { buy, getOrderByCode, history }